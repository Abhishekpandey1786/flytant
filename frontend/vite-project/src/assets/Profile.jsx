...
const Profile = () => {
  ...
  const handleInputChange = (e) => {
    ...
    const finalValue =
      name === "budget" || name === "followers" ? Number(value) : value;
    ...
    try {
      ...
      if (avatarFile) {
        ...
        if (!avatarUpdateResponse.ok) {
          throw new Error(avatarUpdateData.msg || "Failed to upload avatar.");
        }
            ...
            {isEditing ? (
              <form onSubmit={handleProfileUpdate} className="space-y-6 sm:space-y-8">
                ...
                <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-4 border-t border-gray-600">
                  ...
                  <button type="button" onClick={() => { setIsEditing(false); setAvatarFile(null); setFormData(user); }} className="px-5 sm:px-6 py-2 bg-gray-600 text-white font-semibold rounded-full shadow-lg hover:bg-gray-700 transition flex items-center justify-center space-x-2">
              ...
              <>
                <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6 pb-6 border-b border-gray-600">
                  <div className="relative flex-shrink-0 group cursor-pointer" onClick={() => fileInputRef.current.click()}>
                    <img
                      ...
                      alt="User Avatar"
                      className="w-32 h-32 sm:w-32 sm:h-32 border-4 rounded-full shadow-lg object-cover neno-button hover:shadow-fuchsia-800/50 border-fuchsia-800 transition group-hover:opacity-80"
                    />
                ...
                {user.userType === "influencer" && (
                  <div className="bg-gray-700 p-4 sm:p-6 rounded-xl shadow-inner border border-gray-600">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {user.instagram && (
                        <div className="flex items-center space-x-3 break-all">
                          <FaInstagram className="text-fuchsia-500" /><span>@{user.instagram}</span>
                        </div>
                      )}
                      {user.youtube && (
                        <div className="flex items-center space-x-3 break-all">
                          <FaYoutube className="text-fuchsia-500" /><span>{user.youtube}</span>
                        </div>
                      )}
                      {user.facebook && (
                        <div className="flex items-center space-x-3 break-all">
                          <FaFacebook className="text-fuchsia-500" /><span>{user.facebook}</span>
                        </div>
                      )}
                      {user.followers && (
                        <div className="flex items-center space-x-3">
                          <FaUser className="text-fuchsia-500" /><span>Followers: {user.followers.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {user.userType === "advertiser" && (
                  <div className="bg-gray-700 p-4 sm:p-6 rounded-xl shadow-inner border border-gray-600">
                    <h2 className="text-lg sm:text-xl font-semibold mb-4 text-white">Advertiser Profile</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {user.businessName && (
                        <div className="flex items-center space-x-3">
                          <FaBuilding className="text-fuchsia-500" /><span>{user.businessName}</span>
                        </div>
                      )}
                      {user.contactPerson && (
                        <div className="flex items-center space-x-3">
                      ...
                      {user.budget && (
                        <div className="flex items-center space-x-3">
                          <FaDollarSign className="text-fuchsia-500" /><span>Budget: ₹{user.budget.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                  </div>

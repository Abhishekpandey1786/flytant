...
const influencers = [p1, p2, p4, p5, p6, p7, p8];
...
export default function SubscriptionPlans() {
  ...
  // ⭐ Load Cashfree Only Once (Real Fix)
  useEffect(() => {
    const loadSDK = async () => {
      if (!cashfreeInstance) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 px-4 sm:px-6 py-10">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ⭐ Payment Button ALWAYS stable — no remount */}
        <button
          onClick={handlePayment}
          disabled={loading}
          className="w-full mt-6 py-3 rounded-xl font-semibold bg-fuchsia-700 text-white shadow-lg flex items-center justify-center disabled:opacity-50"
        >
          {loading && <Spinner />}

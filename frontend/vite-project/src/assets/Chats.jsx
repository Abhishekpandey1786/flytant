...
export default function Chats() {
    ...
    const handleMessage = (msg) => {
      ...
      if (msg.roomId === currentRoomId) {
        ...
        if (msg.sender !== user._id) {
          api.put(`/chats/read-messages/${currentRoomId}`).catch(err => console.log(err));
        }
      }
      ...
      setUsers((prev) => {
        ...
        const targetUser = prev.find((u) => u._id === otherId);
        if (!targetUser) return prev;
        return [targetUser, ...prev.filter((u) => u._id !== otherId)];
      });
    };
  ...
  useEffect(() => {
    ...
    api.put(`/chats/read-messages/${roomId}`).catch(err => console.log(err));
    ...
    <div className="flex h-[85vh] rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 shadow-2xl">
            ...
            >
              <div className="relative">
                <img
                  ...
                  alt=""
                  className={`w-12 h-12 rounded-full object-cover border ${activeChat?._id === u._id ? "border-fuchsia-500" : "border-slate-700"}`}
                />
                {/* WHATSAPP STYLE BLINK */}
                {unread[u._id] && (
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`font-semibold truncate ${unread[u._id] ? "text-fuchsia-400" : "text-white"}`}>
                  {u.name || u.businessName}
                </h3>
                <p className={`text-xs truncate ${unread[u._id] ? "text-slate-200 font-bold" : "text-slate-500"}`}>
                  {unread[u._id] ? "New message received" : "Tap to chat"}
                </p>
              </div>
            </div>
          ))}
      ...
      <div className={`flex-1 flex flex-col bg-slate-950 ${activeChat ? "flex" : "hidden md:flex"}`}>
        {activeChat ? (
          <>
            {/* HEADER */}
            <div className="flex items-center gap-3 p-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md">
              <button onClick={() => setActiveChat(null)} className="md:hidden text-white mr-2"><FaArrowLeft /></button>
              <img src={activeChat.avatar || activeChat.logo || "https://placehold.co/40"} alt="" className="w-10 h-10 rounded-full object-cover border border-fuchsia-500" />
              <h2 className="text-white font-bold">{activeChat.name || activeChat.businessName}</h2>
            </div>

            {/* MESSAGES */}
            <div ref={boxRef} className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
              {loading ? (
                <div className="text-center text-slate-600 mt-10">Loading...</div>
              ) : messages.length === 0 ? (
            ...
            <form onSubmit={send} className="p-4 bg-slate-900 border-t border-slate-800 flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-950 border border-slate-700 focus:border-fuchsia-500 outline-none rounded-xl px-4 py-3 text-white transition-all"
              />
              <button type="submit" disabled={!text.trim()} className="bg-fuchsia-600 hover:bg-fuchsia-500 disabled:opacity-50 text-white px-6 rounded-xl transition-all">
                <FaPaperPlane />
              </button>
            </form>
          </>
